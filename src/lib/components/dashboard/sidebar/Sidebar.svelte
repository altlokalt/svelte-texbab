<script>
	import SideBarIcon from '$lib/components/dashboard/sidebar/SideBarIcon.svelte';
...
>
	...
	{#if $page.data.user?.role.includes('admin')}
		...
		<a href="/dashboard/inventory"
			><SideBarIcon tooltip="inventory" icon="material-symbols:inventory-sharp" /></a
		>
		<a href="/dashboard/guides"><SideBarIcon tooltip="Guides" icon="bx:bx-book-bookmark" /></a>
		<a href="/dashboard/messages"
			><SideBarIcon tooltip="Messages" icon="bx:bx-message-square-detail" /></a
		>
		<a href="/dashboard/statistics"
			><SideBarIcon tooltip="Statistics" icon="bx:bx-bar-chart-alt-2" /></a
		>
	{/if}
	{#if $page.data.user?.role.includes('chef')}
		<a href="/dashboard/admin/chef"
			><SideBarIcon tooltip="Chef" icon="icon-park-outline:chef-hat-one" /></a
		>
	{/if}
