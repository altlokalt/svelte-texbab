...
COPY . .
RUN turbo prune svelte-texbab --docker
...
RUN NODE_OPTIONS="--max-old-space-size=8192" yarn turbo run build --filter=svelte-texbab --concurrency 2
...
COPY --from=installer /app/apps/svelte-texbab/package.json .
COPY --from=installer /app/apps/svelte-texbab/build /app/build
